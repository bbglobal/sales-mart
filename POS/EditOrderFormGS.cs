...
namespace POS
{
    ...
    public partial class EditOrderFormGS : Form
    {
        Image DeleteImage;
        Image AddImage;
        ...
        public EditOrderFormGS(string billId)
        {
            InitializeComponent();
            _billId = billId;
        ...
        private void LoadOrderItems()
        {
            ...
            // Query to fetch JSON data from bill_list table
            string billListQuery = "SELECT items FROM bill_list WHERE bill_id = @BillId";
            string productPriceQuery = "SELECT selling_price_tax FROM items WHERE item_name = @ItemName";
        ...
        private void OrderCancelButton_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                ...
                if (int.TryParse(BillIDLabel.Text, out billId))
                {
                    ...
                    AdjustStockForCancellationAsync(billId);
                    this.Close();
                }
                else
                {
        ...
        private async void save_button_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Prepare the list of updated items and track stock adjustments
                List<string> itemsToUpdate = new List<string>();
                Dictionary<string, int> stockAdjustments = new Dictionary<string, int>();
                ...
                foreach (DataGridViewRow row in OrderItemsDataGrid.Rows)
                {
                    if (row.Cells["Items"].Value != null && row.Cells["Quantity"].Value != null)
                    {
                        ...
                        // Get the original quantity of the item from the database
                        int originalQuantity = await GetOriginalItemQuantityAsync(billId, itemName);

                        // Determine stock adjustment (add or deduct based on quantity change)
                        int adjustment = originalQuantity - newQuantity;

                        if (adjustment != 0)
                        {
                            if (!stockAdjustments.ContainsKey(itemName))
                                stockAdjustments[itemName] = 0;

                            stockAdjustments[itemName] += adjustment;
                        }

                        // Format the item as "ItemName-Quantity"
                        string formattedItem = $"{itemName}-{newQuantity}";
                ...
                if (allItemsZero && totalAmount == 0)
                {
                    ...
                    if (result == DialogResult.Yes)
                    {
                        // Add all original quantities back to stock
                        await AdjustStockForCancellationAsync(billId);
                ...
                {
                    await UpdateStockAsync(adjustment.Key, adjustment.Value);
                }
        ...
        WHERE bill_id = @billId";
                ...
                LogActivity("Order update", $"Order with {billId} was updated.", Session.Username);
                this.Close();
            }
            catch (Exception ex)
            {
        ...
        // Helper to get the original item quantity
        private async Task<int> GetOriginalItemQuantityAsync(int billId, string itemName)
        {
            string query = "SELECT items FROM bill_list WHERE bill_id = @billId";
            using (SqlCommand cmd = new SqlCommand(query, connection))
            {
                cmd.Parameters.AddWithValue("@billId", billId);

                await connection.OpenAsync();
                string serializedItems = (string)await cmd.ExecuteScalarAsync();
                connection.Close();

                var items = System.Text.Json.JsonSerializer.Deserialize<List<string>>(serializedItems);
                string match = items.FirstOrDefault(i => i.StartsWith($"{itemName}-"));

                return match != null ? int.Parse(match.Split('-')[1]) : 0;
            }
        }

        // Helper to adjust stock for cancellation
        private async Task AdjustStockForCancellationAsync(int billId)
        {
            string query = "SELECT items FROM bill_list WHERE bill_id = @billId";
            using (SqlCommand cmd = new SqlCommand(query, connection))
            {
                ...
                foreach (var item in items)
                {
                    ...
                    await UpdateStockAsync(itemName, quantity); // Add back to stock
                }
            }
        }

        // Helper to update stock
        private async Task UpdateStockAsync(string itemName, int adjustment)
        {
            string query = "UPDATE items SET quantity = quantity + @adjustment WHERE item_name = @itemName";
            using (SqlCommand cmd = new SqlCommand(query, connection))
            {
