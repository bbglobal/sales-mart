...
namespace POS
{
    public partial class SupplierForm : Form
    {
        ...
        public SupplierForm(int rowIndex = -1)
        {
            ...
            if (this.rowIndex != -1)
            {
                Title_label.Text = "Edit Supplier";
        ...
        private void SaveData()
        {
            ...
            try
            {
                ...
                if (rowIndex == -1)
                {
                    string query = "INSERT INTO suppliers (image,supplier_name,email, phone, address,or_image) VALUES (@Image,@Name, @Email, @Phone, @Address, @OR_Image)";
                ...
                {
                    string query = "UPDATE suppliers SET image=@Image,supplier_name=@Name,email = @Email, phone=@Phone, address=@Address, or_image=@OR_Image WHERE id=@Id";
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        command.Parameters.AddWithValue("@Name", Name_TextBox.Text);
                        command.Parameters.AddWithValue("@Email", Email_TextBox.Text);
        ...
        private void SetFields(int rowNo)
        {
            ...
            {
                connection.Open();
                string query = $"select * from suppliers where id={rowNo}";
                ...
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    ...
                    while (reader.Read())
                    {
                        Name_TextBox.Text = (string)reader["supplier_name"];
