...
namespace POS
{
    public partial class ItemsCategoryForm : Form
    {
        ...
        public ItemsCategoryForm(int rowIndex = -1)
        {
            ...
            if (this.rowIndex != -1)
            {
                Title_label.Text = "Edit Item Category Types";
        ...
        private void SaveData()
        {
            ...
            try
            {
                ...
                if (rowIndex == -1)
                {
                    string query = "INSERT INTO item_category (types) VALUES (@Types)";
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        if (rowsAffected > 0)
                        {
                            MessageBox.Show("Item Category Added Successfully", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            ClearFields();
                        }
                    }
                ...
                {
                    string query = "UPDATE item_category SET types=@Types WHERE id=@Id";
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        command.Parameters.AddWithValue("@Types", CategoryTypes_TextBox.Text);
                        ...
                        if (rowsAffected > 0)
                        {
                            MessageBox.Show("Item Category Updated Successfully", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                    }
        ...
        private void SetFields(int rowNo)
        {
            ...
            {
                connection.Open();
                string query = $"select * from item_category where id={rowNo}";
